...
class activity_templateGeneratorHelper extends BaseActivity_templateGeneratorHelper {
    ...
    public function linkToNew($params) {
        return '<li class="sf_admin_action_new">' . button_to(__($params['label'], array(), 'sf_admin'), "/activity_template#", array("class" => "new")) . '</li>';
    }

    public function linkToEdit($object, $params) {
        return '<li class="sf_admin_action_edit">' . link_to(__('<img src="/images/icons/14x14/edit.png" title="Edit" alt="Edit">', array(), 'sf_admin'), "/activity_template#", array("popup_url" => "/activity_template/" . $object->getId() . "/edit")) . '</li>';
    }
    ...
    public function linkToDelete($object, $params, $is_form_action = false) {
        ...
        if (!$is_form_action) {
            ...
            return '<li class="sf_admin_form_action_delete">' . button_to(__($params['label'], array(), 'sf_admin'), "/activity_template#", array('method' => 'delete', 'confirm' => !empty($params['confirm']) ? __($params['confirm'], array(), 'sf_admin') : $params['confirm'])) . '</li>';
        }
    }

    public function linkToCancel($object, $params) {
        return '<input class="cancel" type="button" ' . '" onclick="document.location.href=\'/activity_template\'" value="Cancel"/>';
    }

    public function linkToDone($object, $params) {
        return '<input class="done" type="button" ' . '" onclick="document.location.href=\'/activity_template\'" value="Done"/>';
    }
