...
class gradebook_scaleGeneratorHelper extends BaseGradebook_scaleGeneratorHelper
{
    ...
    {
        return '<li class="sf_admin_action_edit">' . link_to(__('<img src="/images/icons/14x14/edit.png" title="Edit" alt="Edit">', array(), 'sf_admin'), "/gradebook_scale#", array("popup_url" => "/gradebook_scale/" . $object->getId() . "/edit")) . '</li>';
    }
    ...
    public function linkToDelete($object, $params, $is_form_action = false)
    {
        ...
        {
            return '<li class="sf_admin_form_action_delete">' . button_to(__($params['label'], array(), 'sf_admin'), "/gradebook_scale#", array('method' => 'delete', 'confirm' => !empty($params['confirm']) ? __($params['confirm'], array(), 'sf_admin') : $params['confirm'])) . '</li>';
        }
    }
    ...
    {
        return '<input class="button" type="button" value="' . __($params['label'], array(), 'sf_admin') . '" onclick="document.location.href=\'/gradebook/grades\';return false"/>';
    }
